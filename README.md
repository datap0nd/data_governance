...
Tools > Flows stores website discovery settings, the discovered report/filter catalog, flow selections, download behavior, filenames, folders, schedules, and execution timings in the local Metronome SQLite database. Repository code contains the generic flow engine and an official MicroStrategy REST adapter. Authentication credentials and report-specific configuration are not stored in Git.

For ASAP, choose the `ASAP · MicroStrategy API` website type and a weekend refresh schedule. The BI desktop opens the configured portal only to establish browser SSO, then exchanges the shared browser session for a short-lived `X-MSTR-AuthToken` with `GET /MicroStrategyLibrary/api/auth/token`. It traverses accessible projects and Shared Reports folders by stable object ID, creates report instances, reads prompt definitions, and follows the documented member endpoints in pages of 200 until every supported option is stored. No menu coordinates, bounding boxes, page labels, report tabs, or rendered option lists are used. Scans run weekly by default and upsert the local catalog. If a later complete scan cannot find an existing report or prompt, Metronome marks it stale instead of deleting it. Flow creation permits only currently discovered entries.

Each ASAP download is one Metronome operation backed by the documented MicroStrategy report workflow: create an instance, answer prompts with the object IDs saved by the latest catalog refresh, retrieve every result page through Data API v2, and close the instance. MicroStrategy does not support a prompted report as one vendor HTTP request, so Metronome keeps that required sequence explicit instead of hiding an unreliable UI fallback. The worker converts a normal grid result to CSV from the returned definition and raw metric values. It fails closed on unsupported prompt types, unresolved nested prompts, cross-tabs, column attributes, shape mismatches, missing IDs, or incomplete paging.

Flows currently exports CSV only. The worker removes ASAP's report-title and blank preamble rows, rewrites a standard UTF-8 comma-delimited CSV, and records the resulting columns and row count before any optional SQL handoff. Append validates normalized CSV columns against the selected target before writing. Replace atomically drops and recreates the selected table from the CSV columns as PostgreSQL `TEXT`, streams the CSV with PostgreSQL `COPY`, and commits only after the full load succeeds. A failed replace rolls the transaction back to the prior table. Because replace changes the schema, existing indexes, constraints, triggers, and table grants are removed on success. Duplicate normalized names, incompatible append columns, PostgreSQL errors, and worker restarts fail closed with a staged diagnostic record.

Automatic execution is controlled from the Flows list with an Active/Inactive switch. The builder stores the daily or weekly schedule but does not activate it. Run history provides an Expanded logs link for each run. New runs retain every progress event, phase timing, downloaded artifact, SQL target and mode, final error, and complete worker traceback in the local SQLite database.
...
Every scan and download records total duration and phase timings. API scans include browser-session authentication, object discovery, and prompt catalog retrieval. Downloads include API authentication, report execution and paging, file transfer, optional transformation, and optional SQL insertion. The UI estimates the next scan and download from the median of up to ten comparable successful operations; before history exists, it clearly labels a conservative fallback.

Metronome queues each run for a worker on the BI desktop. Every flow has a Browser mode setting because browser SSO is still required. Headless runs use the resident `MXFlowsWorker` Windows service and are appropriate for routine and scheduled downloads. Headed runs start the on-demand `Metronome_Flows_Headed` interactive task and expose Edge for SSO or API diagnostics; report selection, prompts, execution, and result retrieval remain REST operations in both modes.
