...
Flows currently exports CSV only. The worker removes ASAP's report-title and blank preamble rows, rewrites a standard UTF-8 comma-delimited CSV, and records the resulting columns and row count before any optional SQL handoff. SQL insertion validates normalized CSV columns against the selected target before writing. Unexpected columns, missing required target columns, duplicate normalized names, or incompatible values fail the run without committing the transaction.
