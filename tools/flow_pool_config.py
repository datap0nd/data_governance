...
def capacity(path: Path, override: int | None = None) -> int:
    if override is not None and not 1 <= override <= 5:
        raise ValueError('Headless capacity must be between 1 and 5.')
    ...
    with closing(sqlite3.connect(uri, uri=True, timeout=30)) as db:
        if override is not None:
            with db:
                ...
                db.execute("INSERT INTO app_settings(key,value) VALUES ('flows_headless_capacity',?) ON CONFLICT(key) DO UPDATE SET value=excluded.value, updated_at=CURRENT_TIMESTAMP", (str(override),))
            return override
        if not db.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='app_settings'").fetchone():
            return 1
        row = db.execute("SELECT value FROM app_settings WHERE key='flows_headless_capacity'").fetchone()
        try:
...
if __name__ == '__main__':
    ...
    print(capacity(args.database, args.capacity))
