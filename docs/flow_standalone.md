...
There is no Generate standalone step. Save/edit normally; renaming also moves the Flow folder to its sanitized name without an ID suffix and refreshes the saved paths. Changes to the contents of an enabled transformation script are detected even when its path stays the same; owner, schedule, source/catalog, recording and shared preference changes also refresh the derived files, and so does updating the Metronome application, because the script always embeds the current execution code. Startup reconciles existing managed Flows. SQLite remains the source of truth: editing these files does not edit or schedule a Flow in Metronome. `configuration` in flow.json describes the saved Flow; `handover.state` indicates whether its script is `current`, `draft` (the Flow cannot run yet) or `error` (the files could not be updated). These are continuity copies, not a database restore format or a backup of run history/data.
...
A Flow without a saved recording, or whose recording is structurally incomplete, still has a Python file and complete descriptive JSON. Its script exits with an explicit draft explanation instead of running. Finish the recording and save the Flow to obtain an executable version; testing is optional, and the script header states whether the active recording was tested with the current settings. The active and latest draft definitions are identified separately; a new draft does not replace the activated recording used for execution until the Flow is saved.
