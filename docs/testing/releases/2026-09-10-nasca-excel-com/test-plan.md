...
- Risk: high for protected workbook handling and SQL input, contained to an OLE payload whose declared filename is modern Excel.
...
- Synthetic OLE-signature fixture and a fake Excel COM application; no production workbook is copied off the work PC.

## Checks

| ID | Action | Expected result | Evidence |
| --- | --- | --- | --- |
| NASCA-01 | Process an OLE-signature `.xlsx` through `_store_completed_download` with a fake Excel COM application. | The original encrypted bytes are preserved; Excel opens read-only with links/events/alerts/macros disabled; UTF-8 CSV is normalized and becomes the SQL-ready artifact. | Focused pytest output. |
