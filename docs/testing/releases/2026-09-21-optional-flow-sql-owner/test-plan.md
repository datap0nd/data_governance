...
| O-05 | Read the Users guidance and owner help with the option at its default; inspect the existing SQL run log wording. | Users guidance describes the optional ownership choice, and committed-only run log remains correct. | `test_users_profile_journey`; `test_sql_owner_help_and_run_log_confirm_only_committed_changes` |
