...
| P-16 | On a database that already holds a user portal site named `Python`, delete the seeded `python_script` site and rerun `database.init_db()`; then create a Python Flow. | No `IntegrityError`; exactly one `python_script` site exists under a collision-safe internal name, the user site is untouched, the hidden site stays out of the catalog and the Flow is created. |
| P-17 | Save a Python Flow with the SQL destination and `output_mode="direct_replace"` (server) and collect the builder payload with SQL chosen while the hidden output-mode select still holds `direct_replace` (UI). | Both save/send `run_folders`: the final CSV stays in the run folder for Retry SQL and is never published to an exact-name path while SQL is the destination. |
...
| W-12 | Run a two-script chain whose second script fails; a chain whose script exceeds the timeout; a chain whose interpreter cannot be spawned (`subprocess.run` raising `OSError`). | Every step that ran leaves a structured progress event: `python_step_complete` (exit code 0, duration, output size, stdout, checksum) for successful steps and exactly one `python_step_failed` (exit code or `None`, error text, stderr, checksum) for the failed step, emitted before the run fails; no `python_complete` follows a failure. |
