...
| A-18 | Queue a plain Python job and one whose row uses `-sheet` with values `T`, `U`; register a worker advertising only the `python_script` adapter, then one that also advertises `python_script_arguments_v1`. | The adapter-only worker claims the plain job but never the one with arguments or values; the current worker claims it. `flow_python.requires_arguments_capability` is False for the plain job and True for the other. |
