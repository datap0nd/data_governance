...
## 2026-09-12 18:02 UTC — legacy worksheet UI assertion

[CI run 34708849118](https://github.com/datap0nd/data_governance/actions/runs/34708849118), head `1b228aa45358b5f69799dd1d33cd3b07efe99436`, completed with **2072 passed, 1 failed, 22 skipped, 14 warnings** in 1143.45s. Frontend contracts/syntax passed. The sole failure was the old static local-file UI contract expecting the removed mandatory worksheet field. It now checks the shared optional checkbox/names field; the browser journeys in that CI run passed and cover default-off, CSV visibility, and exact-name persistence. Portal-only control assertions remain.

The focused retest passed **1 test**, zero skips, 0.54s pytest time, plus syntax, against that head plus the assertion correction: `tools/check.ps1 -Mode Verify -TestPath tests/test_flow_local_file.py::test_file_builder_exposes_conditional_sheet_and_portal_only_controls -SyntaxPath tests/test_flow_local_file.py`. Evidence: `20260912T180244450Z-41336-22d02fa3`. No application code changed. Final-head CI must still pass and be recorded in the PR before merge.
