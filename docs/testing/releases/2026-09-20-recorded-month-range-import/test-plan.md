...
- Change/PR: pending; import one clear Playwright-recorded noUi month handle as a complete month-range action.
...
Accept when the focused checks pass, the bounded diff review finds no unsafe inference path, and final-head CI passes. The detection must require exactly one noUi touch-area click plus nearby unambiguous month wording. Missing wording, weekly wording or multiple noUi clicks must leave the recording unchanged. Rollback is a PR revert; existing recordings are not rewritten. Isolated verifier artifacts remain ignored, and no protected live evidence is committed.
