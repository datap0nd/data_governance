...
| HF-05 | Make run_flow.py unavailable/modified in the test folder; save again. Restore access or rename the modified copy and save/restart. | DB save survives; Flow files reports error/modified. Restore an unavailable share before saving again: status must identify the older files as stale without changing them. No user edits overwritten. Missing generated script is recreated; startup/save restores current state. Edit README.md and requirements.txt in the fixture, then save a changed Flow: their prior contents must be retained under Scripts/versions. Keep the preserved manual copy. |
