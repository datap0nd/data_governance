...
| C-09 | Codex finding: `resolve_parameters` for *Current week* and *Previous week* at Sunday 24 May 2026 04:30 Dubai on a control with the default (Sunday) convention, an explicit Sunday date control and a Monday control; the same on a Wednesday. | Sunday-start controls give `2026-W22`/`2026-W21` (the week has turned), the Monday control `2026-W21`/`2026-W20`; midweek both conventions agree. |
...
| P-13 | Codex finding: a control that publishes each new value 120 ms after the key press, fixed start `2026-W20` and newest selectable end. | The End probe waits for the value to hold still and confirms it with a second press; handles read `202620`/`202633` and the newest week is `2026-W33`, never the stale `202630`. |
| P-14 | The upper handle declares `aria-valuemax="202640"` while its End value is `202633`; then a matching declaration. | Fails closed naming the shown and declared values; the matching declaration passes. |
| P-15 | `read_extreme` on a handle whose value changes on every read (settle attempts shortened). | Fails with "did not settle". |
...
| ID | Actions | Expected result / evidence |
| --- | --- | --- |
| E-01 | Open `slider.html`, select `Click “Week end”`, open Advanced, enable **This is a date range control**. | The card reads "Set week range" with a **Date range** badge; **This is a range step** becomes disabled; Options show Control values, Week starts on (Sunday), Element box, Start (portal default) and End (newest selectable week, weeks to add 0) with parameter names `start`/`end`. |
| E-02 | Start behavior → Fixed week; enter `2026-W1` then `2026-W01`; end weeks to add `-1`; Element box 2; Week starts on → Monday; Control values → Dates; Start format → `YYYYWW`; rename End to `last_week`; rename Start to `last_week`. | The malformed week is refused beside the actions and the valid one accepted; the week convention survives the kind change; the fixed value converts to `202601`; the rename applies; the duplicate name is refused as not unique. |
| E-03 | Uncheck the control; check it again; set a fixed start and end offset; Save draft. | The recorded click returns (range step enabled again); defaults return; the saved definition validates at version 4 with `goto`, `set_range`, `download`, a week-kind Sunday-start contract with one parent level and the recorded click as source, and the two parameters exactly as entered. |
...
| E-07 | Codex finding on PR #135 (merged before the review arrived): select a value step whose date behavior is *Fixed date*, change **Entered value**, then change **Fixed date**; save. Duplicate the fixed-date step and edit the copy's entered value; save. Edit the entered value of a step whose behavior is *Portal default*. | Each edit reaches the other field: the parameter's fixed value follows the entered value and the entered value follows the fixed date, so the saved `args` and `parameters[...].value` agree (the copy saves `start_2` with the edited value); a portal-default parameter gains no value. |
