...
const controls = {'#flow-download-parallelism':{disabled:true}, '#flow-download-parallelism-help':{}, '#flow-browser-mode':{value:'headless'}};
const folder = {$: id => controls[id]};
vm.createContext(folder);
vm.runInContext(source.slice(source.indexOf('function _flowSyncParallelism('), source.indexOf('function _flowOutlookBuilderHtml(')), folder);
folder._flowSyncParallelism();
assert.equal(controls['#flow-download-parallelism'].disabled, false);
assert.doesNotMatch(controls['#flow-download-parallelism-help'].textContent, /Adopt/);
controls['#flow-browser-mode'].value='headed';
folder._flowSyncParallelism();
assert.match(controls['#flow-download-parallelism-help'].textContent, /visible slots/);
