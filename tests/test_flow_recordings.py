...
def test_recording_uses_capacity_and_requires_capable_visible_worker(flow_db,monkeypatch):
    ...
    def register(identity,recorder=False):
        flows.register_worker(flows.WorkerRegister(worker_id=identity,display_name=identity,
            capabilities={'headed':True,'browser_switch_v1':True,'flow_recorder_v1':recorder,'recorded_flows_v2':recorder,'recorded_flows_v3':recorder,'recorded_flows_v4':recorder,'recorded_flows_v5':recorder,'recorded_validation_engine_v1':recorder,'flow_recorder_controls_v1':recorder,'process_id':123}))
    register('old')
...
def test_cancellation_preserves_catalog_status_and_fences_late_worker(flow_db,monkeypatch):
    ...
    flows.register_worker(flows.WorkerRegister(worker_id='new',display_name='new',capabilities={
        'headed':True,'browser_switch_v1':True,'flow_recorder_v1':True,'recorded_flows_v2':True,'recorded_flows_v3':True,'recorded_flows_v4':True,'recorded_flows_v5':True,'recorded_validation_engine_v1':True,'flow_recorder_controls_v1':True,'process_id':123}))
    flows.claim_run('new')
    with database.get_db() as db:
...
def test_ui_repairs_a_recycled_gscm_row_as_an_exact_bookmark_target():
    from playwright.sync_api import sync_playwright
    root = Path(__file__).resolve().parents[1]
    value = definition(); value['adapter'] = 'gscm_portal'
    click = next(item for item in flow_recording.walk_steps(value['steps']) if item['action'] == 'click')
    click['locator'] = [{'method':'locator','args':['#mainframe\\.Setting0\\.form\\.div_favorite\\.form\\.grd_bookmark\\.body\\.gridrow_7'],'kwargs':{}}]
    click['repair_reason'] = 'This GSCM grid cell is recycled.'
    data = {'flow': {'name': 'GSCM sales', 'source_adapter': 'gscm_portal'}, 'sessions': [],
    ...
    with sync_playwright() as playwright:
        ...
        page.locator('[data-bookmark-name]').fill('Saved report')
        page.get_by_role('button', name='Save draft', exact=True).click()
        ...
        assert 'repair_reason' not in repaired
        browser.close()
