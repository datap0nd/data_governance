...
def _slider_page(values, current, *, handles=2):
    ...
    for index, value in enumerate(current[:handles]):
        ...
        markup.append(f'<div role="slider" tabindex="0" aria-label="{name}"{text} '
                      'style="display:inline-block;width:16px;height:16px;background:#888"></div>')
    markup.append("</div><script>")
    markup.append(f"const values={json.dumps(values)};")
    markup.append('''const handles=[...document.querySelectorAll('[role=slider]')];
      const index=h=>values.indexOf(h.getAttribute('aria-valuetext'));
      handles.forEach((h,i)=>h.addEventListener('keydown',e=>{
        ...
        h.setAttribute('aria-valuetext',values[next]);
      }));</script>''')
    return "".join(markup)
