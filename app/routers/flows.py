...
def standalone_status(flow_id: int):
    ...
    try:
        ...
        if result.get('state') in {'current', 'draft'} and (
            ...
            return {'state': 'modified', 'message': 'Saved Python file is missing or was edited. Save the Flow again: an edited copy is archived under Scripts/versions and run_flow.py is refreshed.'}
...
def update_flow(flow_id: int, body: FlowWrite, request: Request):
    ...
    from app.flow_folder_rename import save_transaction
    with save_transaction(get_db, flow_id) as (db, folder_save):
        existing = db.execute(
        ...
        if body.execution_method == 'recorded' and (body.recording_revision_id or existing['recording_revision_id']):
            # Reject invalid settings or an unrunnable recording before importing
            # files or changing settings. Testing is optional and never checked here.
        ...
        if body.enabled and body.execution_method == 'recorded':
            _build_job(db, flow_id)
        folder_save.rename(db, body.name)
        from app.freshness_inheritance import reconcile_file_binding, reconcile_source
        reconcile_file_binding(db, flow_id)
    ...
    with get_db() as db:
        ...
        def worker_can_claim(candidate) -> bool:
            ...
            return (
                ...
                and (job.get('recording', {}).get('definition', {}).get('version', 1) < 3 or capabilities.get('recorded_flows_v3'))
                and (job.get('recording', {}).get('definition', {}).get('version', 1) < 4 or capabilities.get('recorded_flows_v4'))
                and (job.get('recording', {}).get('definition', {}).get('version', 1) < 5 or capabilities.get('recorded_flows_v5'))
        ...
        if not row:
            ...
            scan = next((candidate for candidate in queued_scans if (
                ...
                and (_loads(candidate['job_json'], {}).get('recording_version', 1) < 3 or capabilities.get('recorded_flows_v3'))
                and (_loads(candidate['job_json'], {}).get('recording_version', 1) < 4 or capabilities.get('recorded_flows_v4'))
                and (_loads(candidate['job_json'], {}).get('recording_version', 1) < 5 or capabilities.get('recorded_flows_v5'))
