...
def build_html_email(recurrence: dict, group: dict, columns: list[str], rows: list[dict[str, str]]) -> str:
    heading_cells = "".join(
        f'<th style="padding:8px 10px;border:1px solid #d6d3cb;background:#efeee9;'
        f'text-align:left;font-size:12px;white-space:nowrap">{html.escape(column)}</th>'
        for column in columns
    )
    body_rows = []
    for row in rows:
        cells = "".join(
            f'<td style="padding:7px 10px;border:1px solid #d6d3cb;font-size:12px;'
    ...
    if recurrence.get("report_url"):
        report_link = (
            f'<p style="margin:4px 0 0"><a href="{html.escape(recurrence["report_url"], quote=True)}" '
            'style="color:#0d7377">Open Power BI report</a></p>'
        )
    return f"""
    <div style="font-family:Segoe UI,Arial,sans-serif;color:#1a1814;line-height:1.4">
      <h2 style="margin:0 0 4px;font-size:20px">{html.escape(recurrence['name'])}</h2>
      <p style="margin:0 0 2px;color:#433f38;font-size:13px">
        {html.escape(group['display_name'])} - {len(rows)} matching row{'s' if len(rows) != 1 else ''}
      </p>
      <p style="margin:0;color:#6b6660;font-size:12px">
        {html.escape(recurrence['report_name'])} / {html.escape(recurrence.get('page_display_name') or recurrence['page_name'])}
        / {html.escape(recurrence.get('visual_title') or recurrence['visual_name'])}
      </p>
      {report_link}
      <div style="margin-top:16px;overflow-x:auto">
        <table cellspacing="0" cellpadding="0" style="border-collapse:collapse;border:1px solid #d6d3cb">
          <thead><tr>{heading_cells}</tr></thead>
          <tbody>{''.join(body_rows)}</tbody>
        </table>
      </div>
      <p style="margin:16px 0 0;color:#928d87;font-size:11px">
        Generated by Metronome at {html.escape(_now().isoformat(timespec='minutes'))} local time.
      </p>
    </div>
    """
