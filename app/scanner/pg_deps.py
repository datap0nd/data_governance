    ...
    *,
    ...
    with get_db() as db:
        try:
            ...
            # Fail closed during an incomplete/older schema migration: without
            # task links we cannot prove that a source is inactive, so leave
            # its operational action untouched and let the next scan retry.
            if "no such table: task_links" in str(exc).casefold():
                return 0
            raise
        source_ids = [
            int(row["source_id"])
