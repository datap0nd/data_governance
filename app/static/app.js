                ...
                <tbody>${flows.map(flow => { const activeRun = runs.find(run => run.flow_id === flow.id && ["queued", "claimed", "running"].includes(run.status)); return `
                    <tr>
                        ...
                        <td class="flow-row-actions">
                            ...
                            ${activeRun && activeRun.job?.execution?.browser_mode === "headed" ? `<button class="btn-sm btn-outline btn-danger-outline flow-stop" data-id="${flow.id}">Stop</button>` : ""}
...
function _flowCatalogHtml(catalog, scans, estimates, workers = []) {
    ...
    return `
        <div class="flow-discovery-summary">
            <div><h2>MicroStrategy API catalog</h2><p>The BI desktop signs in once, exchanges that browser session for a short-lived API token, and reads report IDs, prompts, and every paged option directly from MicroStrategy. No page layout detection is used.</p></div>
        ...
        <div class="flow-catalog-grid">
            <section><div class="flow-panel-head"><div><h2>Websites</h2><p>Browser SSO, API catalog access, and weekly refresh schedule.</p></div><button class="btn-secondary" id="flow-add-site">Add website</button></div>
            ...
            <section><div class="flow-panel-head"><div><h2>Discovered reports</h2><p>Read-only catalog from the latest successful scan.</p></div></div>
                ${catalog.reports.length ? `<div class="flow-catalog-list">${catalog.reports.map(report => `<div class="flow-catalog-row"><span><strong>${esc(report.name)}</strong><small>${report.filters.filter(filter => filter.enabled && !filter.stale).length} API prompts · ${esc(report.automation?.project_name || "MicroStrategy")}</small><small>${esc((report.automation?.category_path || []).join(" > "))}</small><small>Object ${esc(report.automation?.object_id || "unknown")}</small></span><span>${report.stale ? "Stale" : `Seen ${esc(timeAgo(report.last_seen_at))}`}</span><button class="btn-sm flow-scan-report" data-id="${report.id}" ${scans.some(scan => activeStatuses.has(scan.status)) ? "disabled" : ""}>Refresh API data</button></div>`).join("")}</div>` : '<p class="flow-inline-empty">No API reports discovered yet.</p>'}
            </section>
        </div>`;
}
...
async function renderFlows() {
    const [catalog, flows, runs, workers, scans, estimates, sqlCatalog] = await Promise.all([
...
function _flowReportDialog(report = null) {
    ...
    const categoryPath = (automation.category_path || []).join(" > ");
    overlay.innerHTML = `<div class="task-modal flow-dialog flow-report-dialog" role="dialog" aria-modal="true" aria-labelledby="flow-report-title"><h2 id="flow-report-title">${report ? "Edit" : "Add"} report</h2><form id="flow-report-form"><div class="flow-form-grid"><label><span>Website</span><select id="flow-report-site">${state.catalog.sites.map(site => `<option value="${site.id}" ${site.id === report?.site_id ? "selected" : ""}>${esc(site.name)}</option>`).join("")}</select></label><label><span>Report name</span><input id="flow-report-name" required value="${esc(report?.name || "")}"></label><label class="flow-span-2"><span>Portal URL</span><input id="flow-report-url" type="url" required value="${esc(report?.report_url || "")}"></label><label class="flow-span-2"><span>ASAP menu path</span><input id="flow-report-path" value="${esc(categoryPath)}" placeholder="Mobile > Installed Base > Installed Base (MENA)"><small>Use the visible portal labels, separated by &gt;.</small></label><label><span>Report tab</span><input id="flow-report-tab" value="${esc(automation.report_tab || report?.ready_text || "")}" placeholder="Export Wizard (Detail)"></label><label><span>Ready text</span><input id="flow-report-ready" value="${esc(report?.ready_text || "")}" placeholder="Export Wizard (Detail)"></label><label><span>Download action</span><input id="flow-report-download" required value="${esc(report?.download_text || "Export CSV")}"></label><label class="flow-check"><input id="flow-report-enabled" type="checkbox" ${report?.enabled !== false ? "checked" : ""}><span>Enabled</span></label></div><div class="flow-filter-editor-head"><div><h3>Report filters</h3><p>Add the visible ASAP control labels and the choices you want available in the flow builder.</p></div><button type="button" class="btn-secondary" id="flow-add-filter">Add filter</button></div><div id="flow-filter-definitions">${_flowFilterRows(report?.filters?.filter(filter => filter.enabled) || [])}</div><div class="flow-form-error" role="alert"></div><div class="flow-builder-actions"><button type="button" class="btn-secondary flow-dialog-cancel">Cancel</button><button type="submit" class="btn-primary">Save report</button></div></form></div>`;
    document.body.appendChild(overlay);
    const close = _flowBindDialog(overlay, restoreFocus);
...
function _bindFlowWorkspace() {
    ...
    $("#flow-scan-empty")?.addEventListener("click", async () => { const site = state.catalog.sites.find(item => item.enabled); if (!site) return; try { await apiPost(`/api/flows/sites/${site.id}/scan`); toast("MicroStrategy API catalog queued"); await navigate("flows"); } catch (err) { toast("Scan not queued: " + err.message); } });
    ...
    document.querySelectorAll(".flow-scan-site").forEach(button => button.onclick = async () => { button.disabled = true; try { await apiPost(`/api/flows/sites/${button.dataset.id}/scan`); toast("MicroStrategy API catalog queued"); await navigate("flows"); } catch (err) { toast("Scan not queued: " + err.message); button.disabled = false; } });
    ...
    $("#flow-browser-mode")?.addEventListener("change", event => {
        ...
        $("#flow-browser-mode-help").textContent = headed
            ? "Opens Edge only for browser SSO and API diagnostics. Report prompts and data still use MicroStrategy REST."
            : "Uses the background Edge session for SSO, then runs the report through MicroStrategy REST.";
