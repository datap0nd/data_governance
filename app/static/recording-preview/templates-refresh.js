...
for (const src of ['/static/users.js?v=1', '/static/app.js?v=74', '/static/flow_recording_editor.js?v=13', '/static/flow_recordings.js?v=4']) {
...
function flow(id,name,site_id,extra={}){
    return {id,name,source_type:'portal',site_id,site_name:site_id===1?'GSCM':'ASAP',source_adapter:site_id===1?'gscm_portal':'asap_portal',report_id:id,execution_method:'recorded',flow_folder:`${site_id===1?'GSCM':'ASAP'}/${name} (id ${id})`,folder_relative:`${site_id===1?'GSCM':'ASAP'}/${name} (id ${id})`,target_folder:null,output_mode:'direct_replace',filename_template:'orders_{export}.xlsx',period_strategy:'none',file_format:'xlsx',schedule_type:'manual',browser_mode:'headed',selections:{},enabled:false,
        sql_handoff_enabled:true,sql_mode:'append',sql_database:'analytics',sql_schema:'public',sql_table:'regional_orders',sql_target_link_status:'confirmed',post_sql_refresh:{mode:'off',views:[]},email_delivery:{enabled:false,recipients:[],subject:null},recording_revision_id:null,last_run_at:null,last_status:null,...extra};
}
const emailOff={enabled:false,recipients:[],subject:null};
function reset(){
    data.flows=[
        flow(7,'Regional orders',1,{recording_revision_id:3,post_sql_refresh:{mode:'automatic',views:[]},email_delivery:{enabled:true,recipients:['ops@example.test','regional.lead@example.test'],subject:null}}),
        flow(8,'Inventory aging',1,{sql_table:'inventory_aging',post_sql_refresh:{mode:'manual',views:[{database:'analytics',schema:'bi',name:'inventory_aging_mv'}]},email_delivery:{enabled:true,recipients:['ops@example.test'],subject:null}}),
        flow(9,'Sell-out weekly',2,{recording_revision_id:6,sql_handoff_enabled:false,email_delivery:{enabled:true,recipients:['planner@example.test'],subject:'Weekly sell-out file'}}),
    ...
    data.runs=[
        {id:41,flow_id:7,flow_name:'Regional orders',status:'failed',trigger_type:'manual',requested_by:'Analyst',worker_id:'headed-1',created_at:'2026-09-07T05:08:00Z',started_at:'2026-09-07T05:08:20Z',finished_at:'2026-09-07T05:20:06Z',error:'Materialized view refresh failed for analytics.bi.orders_by_region_mv: PostgreSQL SQLSTATE 55P03: canceling statement due to lock timeout SQL insertion had already committed; use Retry view refresh to finish the remaining views.',
            ...
            email:{status:null,detail:null,dispatch_id:null,recipients:['ops@example.test','regional.lead@example.test'],subject:null,files:[],label:'not attempted yet'}},
        {id:40,flow_id:8,flow_name:'Inventory aging',status:'succeeded',trigger_type:'scheduled',requested_by:null,worker_id:'headed-1',created_at:'2026-09-07T04:00:00Z',started_at:'2026-09-07T04:00:10Z',finished_at:'2026-09-07T04:06:40Z',error:null,job:job(40,{flow:{id:8,name:'Inventory aging',execution_method:'recorded',source_type:'portal'},post_sql_refresh:{mode:'manual',discovered_at:'2026-09-07T04:00:00Z',views:[{database:'analytics',schema:'bi',name:'inventory_aging_mv'}]}}),progress:{stage:'complete',message:'Saved the full 1-export bundle and committed 412 row(s) to analytics.public.inventory_aging. Refreshed 1 materialized view(s).'},artifacts:[{file_path:'x',filename:'aging_1.xlsx',status:'saved'}],timings:[{phase:'sql_insertion',duration_ms:2100,status:'succeeded'},{phase:'view_refresh',duration_ms:15200,status:'succeeded'},{phase:'total',duration_ms:390000,status:'succeeded'}],sql_outcome:{committed:true,rows_written:412,files_loaded:1},view_refresh:{mode:'manual',deferred_to_pipeline:false,total:1,completed:1,views:[{sequence_no:1,database:'analytics',schema:'bi',name:'inventory_aging_mv',status:'succeeded',duration_ms:15200,error:null,finished_at:'2026-09-07T04:06:30Z'}],sql_committed:true,retry:null},events:[{id:1,status:'succeeded',stage:'email_delivery',message:'Email with 1 file(s) handed to Outlook for 1 recipient(s).',details:{email_status:'pending',dispatch_id:301,files:['aging_1.csv'],attached:true},created_at:'2026-09-07T04:06:41Z'}],files:[],downloads:null,
            email:{status:'submitted',detail:null,dispatch_id:301,recipients:['ops@example.test'],subject:null,files:['aging_1.csv'],label:'submitted by Outlook'}},
        {id:39,flow_id:9,flow_name:'Sell-out weekly',status:'succeeded',trigger_type:'manual',requested_by:'Analyst',worker_id:'headed-1',created_at:'2026-09-06T04:00:00Z',started_at:'2026-09-06T04:00:10Z',finished_at:'2026-09-06T04:03:00Z',error:null,job:{flow:{id:9,name:'Sell-out weekly',execution_method:'recorded',source_type:'portal'},execution:{browser_mode:'headed'},downloads:{periods:[null]},report:{export_views:[]},transformation:{enabled:false},sql_handoff:{enabled:false},post_sql_refresh:{mode:'off',views:[]}},progress:{stage:'complete',message:'Saved 1 XLSX export(s).'},artifacts:[{file_path:'x',filename:'sellout.xlsx',status:'saved'}],timings:[{phase:'total',duration_ms:170000,status:'succeeded'}],sql_outcome:null,view_refresh:null,events:[{id:1,status:'succeeded',stage:'email_failed',message:'Email could not be handed to Outlook: Failed to launch Outlook email task: Access is denied.',details:{email_status:'failed'},error:'Failed to launch Outlook email task: Access is denied.',created_at:'2026-09-06T04:03:01Z'}],files:[],downloads:null,
            email:{status:'failed',detail:'Failed to launch Outlook email task: Access is denied.',dispatch_id:null,recipients:['planner@example.test'],subject:'Weekly sell-out file',files:['sellout.csv'],label:'not sent'}},
        {id:38,flow_id:8,flow_name:'Inventory aging',status:'succeeded',trigger_type:'scheduled',requested_by:null,worker_id:'headed-1',created_at:'2026-09-06T04:00:00Z',started_at:'2026-09-06T04:00:10Z',finished_at:'2026-09-06T04:07:00Z',error:null,job:{flow:{id:8,name:'Inventory aging',execution_method:'recorded',source_type:'portal'},execution:{browser_mode:'headed'},downloads:{periods:[null]},report:{export_views:[]},transformation:{enabled:false},sql_handoff:{enabled:true,mode:'append',database:'analytics',schema:'public',table:'inventory_aging'},post_sql_refresh:{mode:'off',views:[]}},progress:{stage:'complete',message:'Saved the full 1-export bundle and committed 98120 row(s) to analytics.public.inventory_aging.'},artifacts:[{file_path:'x',filename:'aging_full.csv',status:'saved',file_size:35861300}],timings:[{phase:'total',duration_ms:420000,status:'succeeded'}],sql_outcome:{committed:true,rows_written:98120,files_loaded:1},view_refresh:null,events:[],files:[],downloads:null,
            email:{status:'submitted',detail:'Attachments skipped: 34.2 MB exceeds the 20 MB limit; the email names the file location.',dispatch_id:298,recipients:['ops@example.test'],subject:null,files:['aging_full.csv'],label:'submitted by Outlook'}},
    ];
...
window.apiPostJson=async(path,body)=>{
    ...
    if(path==='/api/flows'){const id=12;const created=flow(id,body.name||'New flow',body.site_id||1,{...body,id,post_sql_refresh:body.post_sql_refresh||{mode:'off',views:[]},email_delivery:body.email_delivery||emailOff});data.flows.push(created);data.revisions[id]=[];data.sessions[id]=[];window.previewPayload=body;status(`Created "${created.name}" in preview (paused).`);return {...created,standalone:{state:'current'}};}
...
window.apiPut=async(path,body)=>{record('PUT',path,body);const id=flowId(path);const target=data.flows.find(f=>f.id===id);if(window.previewFail)throw Error('Save failed. Your changes are kept.');
    if(body.email_delivery?.enabled&&!(body.email_delivery.recipients||[]).length)throw Object.assign(Error('Enter at least one recipient email address for the Email step.'),{validation:[{loc:['body','email_delivery'],msg:'Enter at least one recipient email address for the Email step.'}]});
    const badAddress=(body.email_delivery?.recipients||[]).find(r=>!/^[^\s@,;]+@[^\s@,;]+\.[^\s@,;]+$/.test(r));
    if(body.email_delivery?.enabled&&badAddress)throw Object.assign(Error(`Invalid email address: ${badAddress}`),{validation:[{loc:['body','email_delivery'],msg:`Invalid email address: ${badAddress}`}]});
    Object.assign(target,body);window.previewPayload=body;status(`Saved "${target.name}" in preview · refresh views: ${body.post_sql_refresh?.mode||'off'}${body.post_sql_refresh?.mode==='manual'?` (${body.post_sql_refresh.views.length} chosen)`:''} · email: ${body.email_delivery?.enabled?`${body.email_delivery.recipients.length} recipient(s)`:'off'}.`);return {...target,standalone:{state:'current'}};};
...
window.addEventListener('message',event=>{
    if(event.data?.type==='retry-views'){const source=data.runs.find(r=>r.id===event.data.runId);queueRetry(source);show('log',44);}
    if(event.data?.type==='resend-email'){
        const run=data.runs.find(r=>r.id===event.data.runId);
        run.email={...run.email,status:'submitted',detail:null,dispatch_id:305};
        run.events=[...run.events,{id:run.events.length+1,status:'succeeded',stage:'email_delivery',message:`Send again: Email with ${run.email.files.length} file(s) handed to Outlook for ${run.email.recipients.length} recipient(s).`,details:{email_status:'pending',dispatch_id:305},created_at:now()}];
        status(`Send again in preview: run #${run.id} handed its file to Outlook again; the receipt says submitted.`);
        show('log',run.id);
    }
});
