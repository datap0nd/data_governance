...
for (const src of ['/static/users.js?v=2', '/static/app.js?v=79', '/static/flow_recording_editor.js?v=13', '/static/flow_recordings.js?v=4']) {
...
function pythonFlow(id,name,extra={}){
    return {id,name,source_type:'python',source_adapter:'python_script',site_id:null,report_id:null,site_name:'Python',python_scripts:[...SCRIPTS],python_script_arguments:[...ARGUMENTS],python_script_values:VALUES.map(list=>[...list]),execution_method:'catalog',classification:'production',
...
function pythonJob(flow,extra={}){
    return {flow:{id:flow.id,name:flow.name,execution_method:'catalog',source_type:'python'},execution:{browser_mode:'headless',required_adapter:'python_script'},downloads:{periods:[null],output_mode:flow.output_mode,filename_template:flow.filename_template},report:{export_views:[]},transformation:{enabled:false},
        ...
        python_source:{enabled:true,scripts:[...flow.python_scripts],arguments:[...(flow.python_script_arguments||flow.python_scripts.map(()=>''))],values:(flow.python_script_values||flow.python_scripts.map(()=>[])).map(list=>[...list]),output_format:flow.file_format,destination:flow.sql_handoff_enabled?'sql':'file',timeout_seconds:3600},...extra};
}
function reset(){
    data.flows=[
