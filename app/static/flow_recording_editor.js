...
window.RecordedFlowEditor = (() => {
    ...
    async function open(flowId, settings=null) {
        ...
        function bindDetails(panel,step,action,root,parameterName) {
            ...
            bind('[data-target-kind]',n=>change(d=>{const node=get(d);if(n.value==='bookmark'){node.bookmark_target={kind:'gscm_favorite',bookmark_name:''};delete node.repair_reason;}else delete node.bookmark_target;},true));
