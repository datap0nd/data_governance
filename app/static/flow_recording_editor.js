    ...
    async function open(flowId, settings=null) {
        ...
        function renderDetails() {
            ...
            panel.innerHTML=`<div class="recording-detail-heading"><h3>Step ${index+1}</h3>${undo.length?'<button class="btn-secondary" data-undo>Undo</button>':''}<button class="btn-secondary" data-done>Done</button>${step===root?`<button class="btn-secondary" data-up aria-label="Move up" ${index===0?'disabled':''}>↑</button><button class="btn-secondary" data-down aria-label="Move down" ${index===draft.steps.length-1?'disabled':''}>↓</button><button class="btn-secondary" data-remove>Remove</button>`:'<button class="btn-secondary" data-parent>Back to group</button>'}</div>
                ...
                ${action.action==='click'?(()=>{const favorite=/grd_bookmark|div_favorite/i.test(JSON.stringify(action.locator||[])),target=action.bookmark_target||{};return `<label>Click target <select data-target-kind>${option('recorded','Recorded element',target.kind?'bookmark':'recorded')}${option('bookmark','Bookmark in GSCM Favorite list',target.kind?'bookmark':'recorded')}</select></label>${target.kind?`<label>Exact bookmark name <input data-bookmark-name value="${h(target.bookmark_name)}" placeholder="Name shown in GSCM" required></label><p class="hint">Use the bookmark text, not a recycled grid-row ID or screen position.</p><label>Stable bookmark ID (only if names collide) <input data-bookmark-id value="${h(target.bookmark_id)}"></label><label>Favorite scope <select data-bookmark-scope>${option('','Current recorded scope',target.scope)}${option('Public','Public',target.scope)}${option('Private','Private',target.scope)}${option('Custom','Custom',target.scope)}</select></label>`:`<p class="hint">${favorite?'Suggested: this click is on the GSCM Favorite list. Choose “Bookmark in GSCM Favorite list” so playback finds the bookmark by its exact name and scrolls the list itself when the row is out of view.':'Use Bookmark in GSCM Favorite list only for a recorded bookmark-selection click.'}</p>`}`;})():''}
