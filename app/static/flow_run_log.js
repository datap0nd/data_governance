...
function render(run) {
    ...
    const canRetrySql = terminal && sql.enabled && savedArtifacts.length > 0 && !run.sql_reconciliation_required && !run.sql_outcome?.committed && (!run.downloads || run.downloads.completed === run.downloads.total);
    ...
    const sqlOutcomeText = !sql.enabled && !sqlRetrySource && !refresh?.source_run_id ? "SQL insertion disabled"
        : sqlOutcome?.committed ? `SQL insertion committed${sqlOutcome.rows_written !== undefined && sqlOutcome.rows_written !== null ? ` · ${sqlOutcome.rows_written} row(s)` : ""}${sqlOutcome.inherited_from_run_id ? ` in run #${sqlOutcome.inherited_from_run_id}` : ""}`
        : sqlOutcome?.committed === false ? "SQL insertion rolled back; nothing was committed"
        : sqlOutcome ? "SQL commit outcome uncertain; reconcile the target before rerunning"
