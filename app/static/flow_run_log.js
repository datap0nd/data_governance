...
function render(run) {
    ...
    const canRetrySql = terminal && sql.enabled && savedArtifacts.length > 0;
    ...
    document.getElementById("flow-run-log").innerHTML = `
        <header class="flow-log-header"><div><a href="/#flows">Back to Flows</a><h1>Run #${run.id}: ${esc(run.flow_name)}</h1><p>Complete diagnostic record captured by Metronome.${sqlRetrySource ? ` SQL-only retry of run #${esc(sqlRetrySource)}; ASAP was not opened.` : ""}</p></div><div class="flow-log-header-actions"><span class="flow-log-status ${esc(run.status)}">${esc(run.status)}</span>${canRetrySql ? '<button class="btn-primary" id="flow-retry-sql">Retry SQL only</button>' : ""}</div></header>
    ...
    if (retryButton) retryButton.onclick = async () => {
        ...
        const consequence = sql.mode === "replace"
            ? `This will drop and recreate ${target} from the saved CSV columns as PostgreSQL TEXT, then replace all rows in one transaction using run #${run.id}. Existing indexes, constraints, triggers, and table grants will be removed.`
