...
TOTAL_CAPACITY_KEY = 'flows_total_capacity'


def total_capacity(db) -> int:
    try:
        return max(1, min(MAX_SLOTS, int(setting(db, TOTAL_CAPACITY_KEY, str(DEFAULT_TOTAL_CAPACITY)))))
...
def capacity(db, mode: str) -> int:
    ...
    default = DEFAULT_HEADED_CAPACITY if mode == 'headed' else DEFAULT_HEADLESS_CAPACITY
    try:
        return max(1, min(MAX_SLOTS, int(setting(db, key, str(default)))))
    except (TypeError, ValueError):
        return default
...
def worker_id(slot: int, mode: str = 'headless') -> str:
    ...
    if type(slot) is not int or not 1 <= slot <= MAX_SLOTS:
        raise ValueError(f'Worker slot must be between 1 and {MAX_SLOTS}.')
...
def assignments(db, mode: str | None = None) -> list[dict]:
    ...
    for row in rows:
        ...
        if mode is None or job.get('execution', {}).get('browser_mode', 'headless') == mode:
    ...
    for task in db.execute("""SELECT t.id,t.worker_id,r.job_json FROM flow_download_tasks t
        ...
        if (mode is None or job.get('execution', {}).get('browser_mode', 'headless') == mode) and task['worker_id'] not in occupied:
...
def can_claim(db, identity: str, mode: str) -> bool:
    ...
    if slot and slot > limit:
        return False
    # Both checks run under the same claim transaction. A coordinator's own
    # download task reuses its reservation; helper tasks consume another one.
    return len(assignments(db, mode)) < limit and len(assignments(db)) < total_capacity(db)


def state(db) -> dict:
    background = _slot_state(db, 'headless')
    visible = _slot_state(db, 'headed')
    return {'max_slots': MAX_SLOTS, 'total_capacity': total_capacity(db), 'active_total': len(assignments(db)),
            'headless_capacity': capacity(db, 'headless'), 'headed_capacity': capacity(db, 'headed'),
