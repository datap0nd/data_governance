...
def config_hash(job, *, legacy=False):
    ...
    for key in ('recording', 'recording_parameters', 'resume', 'sql_retry', 'view_retry', 'job_type', 'post_sql_refresh', 'email_delivery'):
        clean.pop(key, None)
...
def attach_job(db, flow, job, *, allow_draft=False):
    ...
    try:
        ...
        raise HTTPException(422, f'This recording cannot run yet: {exc}') from exc
    from app.flow_portable import freeze_transformation
    try:
        source = freeze_transformation(job)
    except (OSError, ValueError) as exc:
        raise HTTPException(409, str(exc)) from exc
    # Testing is optional evidence, never a gate. A run always uses the current
    # settings, the current transformation bytes and the current application
    # code, whether or not the selected version was tested with them. The
    # tested flag records whether its evidence matches exactly this
    # configuration so run logs and scripts can say so.
    ...
    job['recording'] = {'revision': revision['id'], 'definition': definition,
                        ...
                        'tested': tested_with(revision, job, source)}
    job['recording_parameters'] = flow_recording.resolve_parameters(definition)


def tested_with(revision, job, transformation_source):
    """Whether a revision's test evidence was produced with exactly this configuration."""
    return bool(revision['status'] == 'validated'
                and revision['config_hash'] in {config_hash(job), config_hash(job, legacy=True)}
                and revision['transformation_source'] == transformation_source)
...
def queue_operation(db, flow_id, operation, actor, *, revision_id=None, pending_settings=None):
    ...
    job = {'schema_version': 1, 'job_type': 'catalog_scan', 'recording_operation': operation,
           'recording_flow_id': flow_id, 'recording_version': 3, 'recorder_controls': 1, 'execution': {'browser_mode': 'headed', 'browser_channel': flow_browser.configured(db)}, 'browser_channel': flow_browser.configured(db),
